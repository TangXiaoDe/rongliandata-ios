...
class WalletWithdrawResultController: BaseViewController
{
    ...
    fileprivate let promptIconTopMargin: CGFloat = 40
    fileprivate let promptIconSize: CGSize = CGSize.init(width: 75, height: 87)
    fileprivate let promptTitleTopMargin: CGFloat = 20
...
extension WalletWithdrawResultController {
    ...
    fileprivate func initialTopPromptView(_ promptView: UIView) -> Void {
        ...
        self.promptTitleLabel.numberOfLines = 2
        ...
        self.promptInfoLabel.isHidden = true
    ...
    fileprivate func initialWithdrawalInfoView(_ infoView: UIView) -> Void {
        ...
        self.withdrawalAmountView.firstLabel.set(text: "申请提现个数", font: UIFont.pingFangSCFont(size: 16), textColor: UIColor.init(hex: 0x666666), alignment: .left)
        self.withdrawalAmountView.firstLabel.snp.remakeConstraints { (make) in
            make.centerY.equalToSuperview()
            make.leading.equalToSuperview().offset(self.lrMargin)
        }
        self.withdrawalAmountView.secondLabel.set(text: nil, font: UIFont.pingFangSCFont(size: 16, weight: .medium), textColor: UIColor.init(hex: 0x333333), alignment: .right)
        ...
        self.withdrawalFeeView.firstLabel.set(text: "手续费", font: UIFont.pingFangSCFont(size: 16), textColor: UIColor.init(hex: 0x666666), alignment: .left)
        self.withdrawalFeeView.firstLabel.snp.remakeConstraints { (make) in
            make.centerY.equalToSuperview()
            make.leading.equalToSuperview().offset(self.lrMargin)
        }
        self.withdrawalFeeView.secondLabel.set(text: nil, font: UIFont.pingFangSCFont(size: 16, weight: .medium), textColor: UIColor.init(hex: 0x333333), alignment: .right)
        ...
        self.withdrawalStatusView.firstLabel.set(text: "提现状态", font: UIFont.pingFangSCFont(size: 16), textColor: UIColor.init(hex: 0x666666), alignment: .left)
        self.withdrawalStatusView.firstLabel.snp.remakeConstraints { (make) in
            make.centerY.equalToSuperview()
            make.leading.equalToSuperview().offset(self.lrMargin)
        }
        self.withdrawalStatusView.secondLabel.set(text: nil, font: UIFont.pingFangSCFont(size: 16, weight: .medium), textColor: UIColor.init(hex: 0x333333), alignment: .right)
        ...
        self.withdrawalDateView.firstLabel.set(text: "申请时间", font: UIFont.pingFangSCFont(size: 16), textColor: UIColor.init(hex: 0x666666), alignment: .left)
        self.withdrawalDateView.firstLabel.snp.remakeConstraints { (make) in
            make.centerY.equalToSuperview()
            make.leading.equalToSuperview().offset(self.lrMargin)
        }
        self.withdrawalDateView.secondLabel.set(text: nil, font: UIFont.pingFangSCFont(size: 16, weight: .medium), textColor: UIColor.init(hex: 0x333333), alignment: .right)
...
//        self.setupAsDemo()
    ...
    fileprivate func setupData(with model: WalletWithdrawResultModel, configModel: WithdrawConfigModel) -> Void {
        ...
        case .applying:
            ...
            self.promptTitleLabel.text = "提现申请已提交\n等待审核" //"提交成功" // 提交成功、提现成功、提现失败、
            self.promptInfoLabel.text = "提现申请已提交，等待审核"  // 提现已成功到账、审核未通过，提现失败，自动返回可提现余额
            self.withdrawalStatusView.secondLabel.text = "审核中"  // 审核中、提现成功、提现失败
            self.withdrawalStatusView.secondLabel.textColor = UIColor.init(hex: 0x4444FF)   // 0x007FFF、0x333333、E68E40
            //self.withdrawalStatusView.secondLabel.backgroundColor = UIColor.init(hex: 0xF2F8FF) // 0xF2F8FF、0xF5F5F5、0xFFFBF7
        case .success:
            // 提现成功
            self.promptIconView.image = UIImage.init(named: "IMG_wallet_tixian_succese")    // IMG_wallet_tixian_succese/IMG_wallet_tixian_fail
        ...
        case .fail:
            ...
            self.promptTitleLabel.text = "提现失败\n审核未通过" // 提交成功、提现成功、提现失败、
            ...
            self.withdrawalStatusView.secondLabel.textColor = UIColor.init(hex: 0xE16940)   // 0x007FFF、0x333333、0xE68E40
