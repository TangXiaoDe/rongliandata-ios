...
class AssetRequestInfo {
    ...
    struct Wallet {
        ...
        static let bzzWithdrawal = RequestInfo<WalletWithdrawResultModel>.init(method: .post, path: "bzz/withdrawal", replaceds: [])
        ...
        static let withdrawal = RequestInfo<WalletWithdrawResultModel>.init(method: .post, path: "withdrawal", replaceds: [])
    }
    /// 充值
    struct Recharge {
        /// 获取收款方式列表
