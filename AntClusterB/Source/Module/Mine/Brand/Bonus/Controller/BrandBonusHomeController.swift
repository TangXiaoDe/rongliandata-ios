...
class BrandBonusHomeController: BaseViewController {
    ...
    /// childVC列表
    fileprivate var childVCList: [BrandBonusListController] = []
...
//        self.navBar.titleLabel.set(text: "FIL", font: UIFont.pingFangSCFont(size: 18, weight: .medium), textColor: UIColor.white, alignment: .center)
    ...
    fileprivate func initialNestView(_ nestView: XDNestScrollContainerView) -> Void {
        let detailViewH: CGFloat = kScreenHeight - kNavigationStatusBarHeight - kBottomHeight
        // 3. detailView
    ...
    fileprivate func initialBottomDetailView(_ bottomView: UIView) -> [UIView] {
        let childViewH: CGFloat = kScreenHeight - kNavigationStatusBarHeight - kBottomHeight
        // 1. horScroll
        ...
        // 2. childs
        var childTableViews: [UIView] = []
        for (index, type) in self.types.enumerated() {
            let childVC = BrandBonusListController.init(type: type, brand_id: self.brand_id ?? 0)
            ...
            childTableViews.append(childVC.view)
...
extension BrandBonusHomeController: EquipmentHomeTitleViewProtocol {
...
extension BrandBonusHomeController: UIScrollViewDelegate {
    /// 滑动结束 回调
...
extension BrandBonusHomeController: XDNestScrollContainerViewProtocol {
    // 当内容可以滚动时会调用
...
extension BrandBonusHomeController: AppHomeNavStatusViewProtocol {
    /// 导航栏左侧按钮点击回调
