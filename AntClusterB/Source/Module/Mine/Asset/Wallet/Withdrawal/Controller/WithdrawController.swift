...
class WithdrawController: BaseViewController {
    ...
    fileprivate let headerView: WithdrawHeaderView = WithdrawHeaderView.init()
    ...
    fileprivate var feeNum: Double = 1
...
//            make.height.equalTo(WithdrawHeaderView.viewHeight)
        }
        // 2. doneBtn
...
//        }
    ...
    fileprivate func couldDone() -> Bool {
        var couldDone = false
        guard let priceText = self.currentText, !priceText.isEmpty, !((Double(priceText) ?? 0.00) > self.limitSingleMaxNum), (Double(priceText) ?? 0.00) >= self.limitSingleMinNum, (Double(priceText) ?? 0.00) > self.feeNum else {
            return false
        }
        couldDone = true
        return couldDone
...
//        self.present(previewVC, animated: false, completion: nil)
    ...
    /// 显示客服联系方式
    fileprivate func showKefuContactAlert() -> Void {
...
extension WithdrawController: WithdrawHeaderViewProtocol {
    ...
    func headerView(_ view: WithdrawHeaderView, didClickedKefu kefuView: UIView) {
        self.showKefuContactAlert()
    }

}
// MARK: - AppHomeNavStatusViewProtocol
extension WithdrawController: AppHomeNavStatusViewProtocol {
...
extension WithdrawController: InputPayPasswordViewProtocol {
